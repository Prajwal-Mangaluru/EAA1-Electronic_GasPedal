...
#if (Number_of_ISRs > 0) && defined isr_WD_Button__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_WD_Button__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_WD_Button;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_WD_Button__INTC_NUMBER, (ISR_MIN_PRI - 2));
    CyIntSetPriority(isr_WD_Button__INTC_NUMBER, ISR_SCALE(2));
    //\PF
	NVIC_INT_ENABLE(isr_WD_Button__INTC_NUMBER);
#elif Number_of_ISRs > 0
#error  The Interrupt isr_WD_Button could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_WD_Button */
